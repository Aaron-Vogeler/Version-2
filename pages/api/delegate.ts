...
export default async function handler(
  ...
  try {
    ...
    const { goal, context, to_number, custom_system_prompt, rolling_summary_prompt, voice_id, tts_debounce_ms, barge_in_cooldown_ms, caller_utterance_flush_ms, hold_check_in_interval_ms, hold_max_check_ins, model, temperature, max_tokens, top_p, reasoning, stream, json_mode } = req.body;
    ...
    const flyResponse = await fetch(flyUrl, {
      ...
      body: JSON.stringify({
        ...
        voiceId: voice_id,  // Custom Telnyx TTS voice ID
